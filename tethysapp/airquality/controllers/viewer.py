...
from ..utilities import GetCommonRequestData, regionGeojson, \
    TimeSeriesModelDataCompute, SliceFromCatalog, SliceFromCatalogForecastUt, getGeoJSONStations, \
    getGeoJSONSofOnetations, getDefaultStation, plotNcFile,Export2DMapAnimation
...
def WMSProxy(request, url):
    ...
    url = url.replace('/wms/?Service', '/wms?Service').replace('.ncml/?FORMAT', '.ncml?FORMAT').replace(
        '.ncml/?SERVICE', '.ncml?SERVICE')
...
def downloadImage(request):
    ...
    completePath = os.path.join(realPath[:-11], 'workspaces', 'app_workspace', 'Animation', 'ImageFolder', imageName)
    content_type = None
    if gif:
...
def Create_GIF_Map_IMAGE(request):
    realPath = os.path.dirname(os.path.abspath(__file__))
    reqhash = hashlib.sha224(json.dumps(request.data, sort_keys=True).encode('utf-8')).hexdigest()
    cachefile = os.path.join(realPath[:-11], 'workspaces', 'app_workspace', 'Animation',"JSONInfo", reqhash + '.json')
    layersDetail = request.data.get('layersDetail')
    fps = int(request.data.get('fps'))
    ...
    if (os.path.exists(cachefile)):
        ...
        JSONFileContent = Export2DMapAnimation(layersDetail,fps)
        with open(cachefile, "w") as f:
            json.dump(JSONFileContent, f)
    alldata={"Image":JSONFileContent["ImagePath"].split('/')[-1]}
    return JsonResponse(alldata)
...
@authentication_classes([])
@permission_classes([])
def SliceFromCatalogForecast(request):
    url = request.data.get('url')
    data_ext = request.data.get('data_ext')
    runDate = request.data.get('runDate')
    data = {}
    try:
        data["data"] = SliceFromCatalogForecastUt(url, data_ext, runDate)
        data["status"] = 200
    except:
        data["status"] = 500
        data["error"] = 'internal Server error'
...
def trueColorLegendImage(request):
    imageName=   request.GET.get('Imagename')
    realPath = os.path.dirname(os.path.abspath(__file__))
    completePath = os.path.join(realPath[:-11], 'workspaces', 'app_workspace', 'LegendImage', imageName)
    content_type = 'image/png'
    zip_file = open(completePath, 'rb').read()
    response = HttpResponse(zip_file, content_type=content_type)
    return response
